...
class New_Gloabl_sum_embedding(torch.nn.Module):
    def __init__(self, config):
        ...
        for idx in range(0, num_layer - 1):
            self.mlp_layers_real.append(torch.nn.Linear(in_features=int(self.mlp_layers[idx]),
                                                        out_features=int(self.mlp_layers[idx + 1])).cuda())
    ...
    def forward(self, user_indices, item_indices, group=True):
        ...
        vector = torch.cat((element_product_tweets, element_product_grouping), 1)  # [b,e]
...
class New_Gloabl_sum_embedding_MLPEngine(Engine):
    ...
    def __init__(self, config):
        ...
        if config['use_cuda'] is True:
            ...
            self.model.cuda()
        super(New_Gloabl_sum_embedding_MLPEngine, self).__init__(config)
        if config['pretrain']:
