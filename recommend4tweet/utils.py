...
def use_optimizer(network, params):
    if params['optimizer'] == 'adam':
        optimizer = torch.optim.Adam([{"params": [y[1] for y in network.named_parameters() if
                                                  ("word_embeddings_lstm" not in y[0]) and (y[1].requires_grad)]},
                                      {"params": [y[1] for y in network.named_parameters() if
                                                  ("word_embeddings_lstm" in y[0]) and (y[1].requires_grad)],
...
# df = pd.read_csv(path, header=0, dtype={0: np.int, 1:str})
