...
class New_Gloabl_sum_embedding(torch.nn.Module):
    def __init__(self, config):
        ...
        self.mlp_layers = eval(config['args'].mlp_layers)
        # self.latent_dim = config['latent_dim_mf']
        self.latent_dim = int(self.mlp_layers[0]/2)
        self.num_friends = config['num_friends']
        self.user_friends = config['user_friends']

        #GMF
        self.embedding_item = torch.nn.Embedding(num_embeddings=self.num_items, embedding_dim=int(self.latent_dim))
        ...
        self.mlp_layers_real= []
        for idx in range(0,num_layer-1):
                self.mlp_layers_real.append(torch.nn.Linear(in_features=int(self.mlp_layers[idx]), out_features=int(self.mlp_layers[idx+1])).cuda())

        self.mlp_layers_real.append(torch.nn.Linear(in_features=int(self.mlp_layers[-1]),out_features=1).cuda())


        self.affine_output = torch.nn.Linear(in_features=self.latent_dim, out_features=1)
        '''
    
        self.three_weight1 = torch.nn.Linear(in_features=self.latent_dim*2, out_features=int(self.latent_dim))
        self.three_weight2 = torch.nn.Linear(in_features=int(self.latent_dim ),
                                             out_features=int(self.latent_dim / 2))
    ...
    def forward(self, user_indices, tweet_indices, item_indices, group=True):
        ...
        vector = torch.cat((element_product_tweet,element_product_grouping),1) # [b,e]

        logits = self.mlp_layers_real[0](vector)
        for weight in self.mlp_layers_real[1:]:
            logits = weight(logits)
...
class New_Gloabl_sum_embeddingMLPEngine(Engine):
    ...
    def __init__(self, config):
        ...
        if config['use_cuda'] is True:
            ...
            self.model.cuda()
        super(New_Gloabl_sum_embeddingMLPEngine, self).__init__(config)
        if config['pretrain']:
